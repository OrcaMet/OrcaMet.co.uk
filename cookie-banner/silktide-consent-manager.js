...
class SilktideConsentManager {
  ...
  _validateConfig(config) {
    ...
    if (!config.consentTypes || !Array.isArray(config.consentTypes)) {
      throw new Error(
        "Silktide Consent Manager: config.consentTypes must be an array",
      );
    }
    if (config.consentTypes.length === 0) {
      throw new Error(
        "Silktide Consent Manager: config.consentTypes cannot be empty",
      );
    }
  }
  ...
  _getLocalStorageItem(key) {
    ...
    try {
      ...
      console.warn(
        "Silktide Consent Manager: Error reading from localStorage",
        e,
      );
      return null;
    }
  ...
  _setLocalStorageItem(key, value) {
    ...
    try {
      ...
      console.warn(
        "Silktide Consent Manager: Error writing to localStorage",
        e,
      );
      return false;
    }
  ...
  _removeLocalStorageItem(key) {
    ...
    try {
      ...
      console.warn(
        "Silktide Consent Manager: Error removing from localStorage",
        e,
      );
      return false;
    }
  ...
  _injectScript(scriptConfig, consentId) {
    ...
    if (!url) {
      console.warn(
        "Silktide Consent Manager: Script URL is required",
        scriptConfig,
      );
      return;
    }
  ...
  batchUpdateConsents(consentStates) {
    ...
    if (this.config.debug) {
      console.log(
        "▶ GTM Event Sent: " + this.config.eventName + " (from user action)",
      );
    }
  ...
  runConsentCallbacksOnLoad() {
    ...
    if (hasGtagUpdates && typeof gtag === "function") {
      ...
      gtagConsentUpdate.analytics_storage &&
      typeof window.silktide === "function"
    ) {
      // Silktide Analytics fallback
      const analyticsGranted =
        gtagConsentUpdate.analytics_storage === "granted";
  ...
  createBanner() {
    ...
    this.prompt = this.createWrapperChild(
  ...
  getModalContent() {
    ...
    const creditLinkText =
      ...
      "Get this consent manager for free";
  ...
  toggleModal(show) {
    ...
    if (show) {
      ...
      const modalCloseButton =
        this.preferences.querySelector(".stcm-modal-close");
...
(function () {
  ...
  function resetConsent() {
    if (!consentManager) {
      console.error(
        "Silktide Consent Manager: Cannot reset - no instance initialized.",
      );
      return;
    }
...
window.silktideConsentManager.init({
  ...
  consentTypes: [
    ...
    /*     {
      ...
      label: "Functional (1)",
      description:
        "<p>These cookies enable <strong>TawkTo live chat</strong> support and remember your conversation with our support team.</p>",
      defaultValue: false,
      onAccept: function () {
      },
    }, */
  ...
  text: {
    prompt: {
      description:
        " <h5>🍪 Hello traveller, it's cookie time!</h5> <br> <p>We use cookies on our site to ensure it functions properly and to provide a smooth, reliable experience.</p>",
...
/* function loadTawkTo() {
  ...
  s1.async = true;
  s1.src = "https://embed.tawk.to/6880f5e021815b1919e9531d/1j0rs0tg0";
  s1.charset = "UTF-8";
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (manager.getConsentChoice("marketing")) {
    loadTawkTo();
  }
}); */
